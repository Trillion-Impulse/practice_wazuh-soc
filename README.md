...
- auth.log
    ...
    > auth.log 스크린샷
    ![auth.log](screenshots/ssh-login-failure/ssh-login-failure-per1min-1-log.png)
- Wazuh
    - 10회 중 가장 마지막 Alert
...
### 이슈 2: 각 vm의 시스템 시간 일치하지 않는 의심 발생
- 상황
    - last login의 다름을 시스템 시간 불일치로 착각
    - 각 vm은 독립적인 운영체제이므로 다를 가능성이 존재
    - SOC 환경에서는 이벤트 발생 시간, 로그 생성 시간, SIEM에서 탐지한 시간이 연결되어야 함
- 해결 과정
    - 각 vm에서 현재 시간을 확인
        - `date`
        - 현재 시간 일치
    - 각 vm에서 시간대 및 NTP 상태 확인
        - `timedatectl`
        - Time Zone 일치
        - NTP 서버 활성화
    > date & timedatectl
...
### 이슈 4: Wazuh Dashboard에 HTTPS로 접속했을 때 브라우저에서 "이 사이트는 보안 연결이 사용되지 않았습니다"라는 보안 경고가 발생
- 원인
    -  Wazuh는 기본 설치 시 공인 CA가 아닌 Self-Signed Certificate를 사용하므로 브라우저가 인증서를 신뢰하지 못해 경고를 표시
- 해결
    - 실습 환경에서 서버를 직접 구축한 것이므로 인증서 정보를 확인한 후 경고를 무시하고 접속
    - 실제 운영 환경에서는 공인 CA 또는 사내 CA 인증서를 적용하여 경고를 제거 가능

### 이슈 5: SSH Brute Force 탐지 규칙 5763발생
- 상황
...
- Pluggable Authentication Modules
    - 인증 기능들을 여러 개의 작은 모듈로 나누어 필요한 것만 조합하는 구조
    - 인증 방식을 쉽게 교체할 수 있는 인증 모듈
- 인증을 대신 처리해주는 공통 인증 시스템
- 리눅스에는 로그인 프로그램이 많음
    - SSH, GUI, sudo, su, ...
    - 만약 각각이 비밀번호 확인 방식을 따로 만든다면 관리가 힘듬
    - PAM이 이들의 인증을 대신 처리해 줌
- PAM의 종류
    - auth
        - 사용자 인증
        - ex: 비밀번호, OTP, 지문
    - account
        - 계정 사용 가능 여부 확인
        - ex: 계정 잠금, 로그인 가능 시간, 만료 여부
    - password
        - 비밀번호 변경
    - session
        - 로그인 시작,종료 시 수행할 작업
        - ex: 홈 디렉터리 생성, 로그 기록, 환경 변수 설정
- PAM의 동작 흐름
    ```
    사용자
      │
    ...
    SSH 서버(sshd)
      │
      │ /etc/pam.d/sshd 설정 확인
      ▼
    PAM
    ├─ auth      → 비밀번호 확인
    ├─ account   → 계정 사용 가능 여부 확인
    ├─ session   → 로그인 환경 준비
...
- 출력 형식 지정
    ...
    - 형식 표
        ...
        | `%B` | 월 이름 (전체) | `June` |
        | `%b` | 월 이름 (축약) | `Jun` |
        | `%T` | `%H:%M:%S`와 동일 | `14:35:20` |
        | `%F` | `%Y-%m-%d`와 동일 | `2026-06-29` |
- 옵션
    - `-u`: UTC(세계 표준시)를 출력
    - `-R`: 이메일 등에서 사용하는 RFC 형식으로 출력
    - `-I`: ISO 8601 형식으로 출력
- 날짜와 시간 변경
    - 관리자 권한 필요
...
- 사용법
    - 아래 한 줄 이동: ↓, Enter
    - 위 한 줄 이동: ↑
    - 아래 한 페이지 이동: Space, Page Down
    - 위 한 페이지 이동: b, Page Up
    - 맨 처음: g
    - 맨 끝: G
    - 종료: q
    - 앞에서부터 검색: `/검색어` + Enter
